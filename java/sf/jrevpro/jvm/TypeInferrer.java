...
public class TypeInferrer implements JVMConstants {
	...
	public static List<String> getArguments(String methodSignature) {
		...
		int argBeginIndex = methodSignature
				.indexOf(JVM_METHOD_ARG_BEGIN_DELIMITER);
		if (argBeginIndex == -1) {
		...
		if (endIndex > (argBeginIndex + 1)) {// at least one argument is present
			methodSignature = methodSignature.substring(argBeginIndex + 1,
					endIndex);
	...
	/**
	 * Both boolean and char are represented as integers . This takes care of
	 * the conversions
	 * 
	 * @param value
	 *            Old Value.
	...
	public static String getValue(String value, String jvmType) {
		...
		if (lastQIndex == -1 || lastColonIndex == -1
	 ...
	 * 
	 * @param type
	 *            Input Type is a JVM Array Type. For eg, "[I" representing
	 *            int[] etc.
	...
	public static boolean doesTypeOccupy2EntriesInVariableTable(String jvmType) {
		return Character.toString(JVMConstants.JVM_TYPE_LONG).equals(jvmType)
				|| Character.toString(JVMConstants.JVM_TYPE_DOUBLE).equals(
						jvmType);
