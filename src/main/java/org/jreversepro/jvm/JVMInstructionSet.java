...
public class JVMInstructionSet implements Opcodes {
  ...
  public static boolean isInvokeIns(int opcode) {
    return opcode == OPCODE_INVOKESPECIAL || opcode == OPCODE_INVOKEVIRTUAL
        || opcode == OPCODE_INVOKESTATIC || opcode == OPCODE_INVOKEINTERFACE;
  }
  ...
  /**
   * In case this instruction is a branch instruction on some condition then the
   * operator corresponding to the operator is returned. For eg, for OPCODE_IFEQ
   * , = is returned.
   * 
   ...
   * datatype onto the JVM stack ) , then that variable index is returned. If no
   * variable is referenced then INVALID_VAR_INDEX is returned.
   * 
  ...
  /**
   * In case this instruction is a store instruction, ( that is popping a local
   * variable from the JVM stack ) , then that variable index is returned. If no
   * variable is referenced then INVALID_VAR_INDEX is returned.
   * 
   * @return index of the variable referred to. INVALID_VAR_INDEX, otherwise.
   */
