...
public class SymbolTable implements VariableTable {
	...
	public void recordLocalDatatypeReference(int localVariableIndex,
		...
		if (ent != null) {
			...
			if (ent.datatype.equals(jvmVariableType)) {
				...
				logger
						.info("Collision Detected. Adding a new local variable index  "
								+ localVariableIndex
								+ " of type "
								+ jvmVariableType);
				addEntry(localVariableIndex, referredBytecodeIndex,
						jvmVariableType);
			...
			logger.info("Adding a new local variable index  "
					+ localVariableIndex + " of type " + jvmVariableType
					+ " with bytecode index " + referredBytecodeIndex);
	...
	public String getName(int aVarIndex, int aInsIndex) {
		...
		if (entry == null) {
			...
			throw new IllegalArgumentException(
					"Does not have any symbol entry for Variable Index:"
							+ aVarIndex + ", " + "instruction index: "
							+ aInsIndex);
		}
	...
	private void addEntry(int localVariableIndex, int storeIndex, String jvmType) {
		...
		SymbolEntry ent = new SymbolEntry(localVariableIndex, storeIndex,
				jvmType, name);
