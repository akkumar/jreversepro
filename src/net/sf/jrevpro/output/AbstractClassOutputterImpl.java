...
abstract class AbstractClassOutputterImpl implements JVMConstants {
	 ...
	 * them. classes belonging to the default package are excluded. Also there
	 * is an option by which we can exclude the classes that belong to a given
	...
	/**
	 * Returns the String Representation of the qualifier. Certain qualifiers
	 * like volatile, transient, sync. are applicable only for methods and
	 * fields. and not classes. To identify them separately, we also pass
	 * another parameter called memberOnly. Only if this is set then those bits
	 * are checked for. Else they are ignored, since for a class/interface they
	 * may not be applicable.
	 * 
	 * @param rhsQualifier
	 *            Qualifier byte with the bits set.
	 * @param memberOnly
	 *            Only if this is set then the bits relevant to fields and
	 *            methods only are checked for. Else ignored.
	...
	protected void outputMethodHeader(Method method) {
		...
		if (method.getName().compareTo(CLINIT) != 0) {
			...
			for (int i = 0; i < args.size(); i++) {
				...
				sb.append(method.getVariableTable().getName(
						baseVariableIndex++,
						VariableTable.FULL_SCOPE_INSTRUCTION_INDEX));

				if (TypeInferrer
						.doesTypeOccupy2EntriesInVariableTable(jvmArgType)) {
	 ...
	 */
	private void outputThrowsClause(List<String> throwsClasses,
			Import importInfo) {
