...
public class DefaultInstructionListParser implements InstructionListParser {
	...
	/**
	 * 
	 * This method is responsible for converting the bytecode stream of the
	 * method into JVM opcodes. By default all JVM opcodes are of fixed length
	 * in all contexts, except some. tableswitch and lookupswitch are basically
	 * variable length opcodes and their length primarily depend on the number
	 * of switch legs in the source code since they are one composite statement
	 * that contains information about all the possible targets.
	 */
	 ...
	 *            stream of the method. The format is as follows. The first 4
	 *            bytes are for defaultbyte. The next 4 for the lowest value in
	 *            the case leg values. The next 4 for the highest value in the
	 *            case high values. Then we will have ( high - low ) * 4 bytes -
	 *            all containing the target index to be jumped into relative to
	 *            the index of the current switch instruction.
	 ...
	 * 
	 * @param index
	 *            beginning index of the lookupswitch statement into the
	 *            bytecode stream of the method. The format is as follows. The
	 *            first 4 bytes are for defaultbyte. The next 4 for the number
	 *            of pairs of ( case leg value, target) that will appear as
	 *            follows, say N. Then there will be (2 * N) * 4 bytes , ( N
	 *            pairs of integers ), with each pair containing the case leg
	 *            value first and the second integer letting us know the target
	 *            of the case leg relative to the index of the method.
	 * @return Returns the integer that is Index + length of the instruction so
	 *         that the next JVM opcode can proceed from there.
	 */
	 ...
	 * 
	 * @param newIndex
	 *            Index from which the integer array starts. This is in
	 *            big-endian order.
