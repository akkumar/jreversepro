...
public class ConstantPool {
	...
	public void addFieldRefEntry(int ptr1, int ptr2) {
		listEntries.add(new ConstantPoolEntry(TAG_FIELDREF, STR_INVALID, ptr1,
				ptr2));
	}
	...
	public void addInterfaceRefEntry(int ptr1, int ptr2) {
		listEntries.add(new ConstantPoolEntry(TAG_INTERFACEREF, STR_INVALID,
				ptr1, ptr2));
	}
	...
	public void addNameTypeEntry(int ptr1, int ptr2) {
		listEntries.add(new ConstantPoolEntry(TAG_NAMETYPE, STR_INVALID, ptr1,
				ptr2));
	}
	...
	public void addClassEntry(int classIndex) {
		listEntries.add(new ConstantPoolEntry(TAG_CLASS, STR_INVALID,
				classIndex, PTR_INVALID));
	}
	...
	public void addStringEntry(int stringIndex) {
		listEntries.add(new ConstantPoolEntry(TAG_STRING, STR_INVALID,
				stringIndex, PTR_INVALID));
	}
	 ...
	 * value of #8 is "MyString", this method on given input 6 returns
	 * "MyString".
	 * 
	 * @param index
	 *            Index to ConstantPool
	...
	/**
	 * Given an index to TAG_NAMETYPE this returns the name of the member (
	 * field / method )
	 * 
	 * @param index
	 *            Index to ConstantPool
	...
	/**
	 * Given an index to TAG_NAMETYPE this returns the type of the member (
	 * field / method )
	 * 
	 * @param index
	 *            Index to ConstantPool
	...
	/**
	 * Given an Cp index this returns the proper constant pool value depending
	 * on the tag type. If TAG_INTEGER, returned as such. Else if TAG_STRING all
	 * the escape characters are properly quoted and returned.
	 * 
	 * @param index
	 *            Index to ConstantPool
	 * @return Returns value of that Cp Entry.
	...
	/**
	 * The constantpool is the one and only source that contains the references
	 * to external types. Hence all the entries are analyzed for all external
	 * types referred to by this class and are consolidated into a class called
	 * Import. This Import can further be used to get the short form of
	 * classname etc.
	 * 
		...
		if (importedClasses != null) {
			...
			for (int i = 0; i < listEntries.size(); i++) {
				...
				case TAG_FIELDREF:
					...
					importedClasses.addClass(TypeInferrer.getJLSType(type,
							true));
					break;
				...
				case TAG_INTERFACEREF:
					...
					importedClasses.addClass(TypeInferrer.getJLSType(
							TypeInferrer.getReturnType(methodType), true));
					break;
				}
			}
			return importedClasses;
	...
	public String getBasicDataTypeValue(int index) {
		...
		case TAG_STRING:
			...
			sb.append(JLSStringEncoder.encodeStringInJLSSource(getFirstDirectName(index))
					+ "\"");
			return sb.toString();
		default:
			return STR_INVALID;
	...
	public String getName(ConstantPoolEntry ent) {
		ConstantPoolEntry entNameType = (ConstantPoolEntry) listEntries.get(ent
				.getPtr2());
		ConstantPoolEntry entName = (ConstantPoolEntry) listEntries
				.get(entNameType.getPtr1());
	...
	public String getType(ConstantPoolEntry ent) {
		ConstantPoolEntry entNameType = (ConstantPoolEntry) listEntries.get(ent
				.getPtr2());
		ConstantPoolEntry entType = (ConstantPoolEntry) listEntries
				.get(entNameType.getPtr2());
	 ...
	 * until it reaches a tag which is not dependent on anything else. Usually
	 * the final tag happens to a TAG_UTF8.
	 * 
	 * @param sb
	...
	private void getSingleEntryInfo(StringBuilder sb, int cpIndex) {
		if (cpIndex >= 1 && cpIndex < listEntries.size()) {
			ConstantPoolEntry ent = (ConstantPoolEntry) listEntries
					.get(cpIndex);
			sb.append("\n" + cpIndex + " : " + ent);
