...
public class SymbolTableLoader implements Opcodes {
   ...
   * SymbolTableContext symContext, InstructionList list) { SymbolTable symTable
   * = new SymbolTable(symContext); logger.fine("Loading Symbol Table
   * **********"); // RuntimeFrame rtf = new RuntimeFrame(pool, symTable,
   ...
   * ins.index, true);
   * 
   * opStack.push(JVMConstants.FOREIGN_OBJ, JVMConstants.FOREIGN_CLASS, VALUE);
   * } else { if (branches.isJSRTarget(ins.index)) {
   * opStack.push(JVMConstants.FOREIGN_OBJ, JVMConstants.FOREIGN_CLASS, VALUE);
   * } if (varIndex != Instruction.INVALID_VAR_INDEX) { // Save the local
   * variable index. symTable.assignDataType(varIndex, opStack.topDatatype(),
   * ins.index, false); } } runtimeFrame.operateStack(ins, opStack); // Add a
   * referenced count of line numbers int referredVar = ins.referredVariable();
   * if (referredVar != Instruction.INVALID_VAR_INDEX) {
   ...
   * OPCODE_GOTO) { branches.addGotoEntry(ins.index, ins.getTargetPc()); } else
   * if (ins.opcode == OPCODE_JSR) { branches.addJSRPc(ins.getTargetPc()); }
   * else if (ins.opcode == OPCODE_RET) { branches.addRetPc(ins.getNextIndex());
   * } else if (ins.opcode == OPCODE_GOTOW) { branches.addGotoEntry(ins.index,
